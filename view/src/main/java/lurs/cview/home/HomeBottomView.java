...
public class HomeBottomView extends LinearLayout {
    ...
    public void showMenu() {
        if (bottomList != null) {
            ...
            for (int i = 0; i < bottomList.size(); i++) {
                ...
                view.tvTitle.setTextColor(getResources().getColor(bottomList.get(i).normalColor));
                view.ivIcon.setImageResource(bottomList.get(i).normalSrc);
                if (i == 0) {
                    view.tvTitle.setTextColor(getResources().getColor(bottomList.get(i).hoverColor));
    ...
    private void refreshView(MenuItemView itemView, int position) {
        ...
        itemView.tvTitle.setTextColor(getResources().getColor(bottomList.get(position).hoverColor));
